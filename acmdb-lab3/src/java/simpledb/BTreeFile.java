...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		while (it.hasNext()) {
			e = it.next();
			if (f == null || f.compare(Op.LESS_THAN_OR_EQ, e.getKey())) {
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		BTreeInternalPage 	pr	= getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
        BTreeLeafPage		np 	= (BTreeLeafPage) getEmptyPage(tid, dirtypages, BTreePageId.LEAF);
        ...
        pr.insertEntry(new BTreeEntry(it.next().getField(keyField), page.getId(), np.getId()));

        page.setParentId(pr.getId());
        np.setParentId(pr.getId());
//		updateParentPointers(tid, dirtypages, pr);
        np.setLeftSiblingId(page.getId());
        np.setRightSiblingId(page.getRightSiblingId());

        if (page.getRightSiblingId() != null) {
        	BTreeLeafPage rp = (BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE);
			rp.setLeftSiblingId(np.getId());
			dirtypages.put(rp.getId(), rp);
        }
        page.setRightSiblingId(np.getId());

        dirtypages.put(page.getId(), page);
        dirtypages.put(pr.getId(), pr);
        dirtypages.put(np.getId(), np);

        return field.compare(Op.LESS_THAN_OR_EQ, page.reverseIterator().next().getField(keyField)) ? page : np;
	}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		BTreeInternalPage 	pr	= getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
		BTreeInternalPage	np 	= (BTreeInternalPage) getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);
		...
		pr.insertEntry(new BTreeEntry(et.getKey(), page.getId(), np.getId()));
		page.deleteKeyAndRightChild(et);

		page.setParentId(pr.getId());
		np.setParentId(pr.getId());
		updateParentPointers(tid, dirtypages, np);

		dirtypages.put(page.getId(), page);
		dirtypages.put(pr.getId(), pr);
		dirtypages.put(np.getId(), np);

		return field.compare(Op.LESS_THAN_OR_EQ, et.getKey()) ? page : np;
	}
...
//		System.out.println(page.getMaxTuples());
//		System.out.println(page.getNumTuples());
//		System.out.println(sibling.getNumTuples());
		for (int i = (sibling.getNumTuples() - page.getNumTuples()) / 2; i > 0; i--) {
			Tuple t = it.next();
			sibling.deleteTuple(t);
			page.insertTuple(t);
		}
		entry.setKey((isRightSibling ? page : sibling).reverseIterator().next().getField(keyField));
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		for (int i = (leftSibling.getNumEntries() - page.getNumEntries()) / 2; i > 0; i--) {
		...
		parentEntry.setKey(leftSibling.reverseIterator().next().getKey());
		leftSibling.deleteKeyAndRightChild(leftSibling.reverseIterator().next());
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		for (int i = (rightSibling.getNumEntries() - page.getNumEntries()) / 2; i > 0; i--) {
			BTreeEntry e = it.next();
		...
		parentEntry.setKey(rightSibling.iterator().next().getKey());
		rightSibling.deleteKeyAndLeftChild(rightSibling.iterator().next());
		parent.updateEntry(parentEntry);
		updateParentPointers(tid, dirtypages, page);
	}
